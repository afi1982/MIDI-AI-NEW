diff --git a/services/midiQualityService.ts b/services/midiQualityService.ts
new file mode 100644
index 0000000000000000000000000000000000000000..8c016e9f07200e96a87c2776f05ce12b9ec2aecd
--- /dev/null
+++ b/services/midiQualityService.ts
@@ -0,0 +1,219 @@
+import { GrooveObject, ChannelKey, NoteEvent } from '../types';
+import { theoryEngine, NOTE_NAMES } from './theoryEngine';
...
+
+
+const resolveRootKey = (rawKey?: string): string => {
+  if (!rawKey) return 'C';
+  const normalized = theoryEngine.normalizeNote(rawKey);
+  if (NOTE_NAMES.includes(normalized)) return normalized;
+  if (rawKey.length) {
+    const first = theoryEngine.normalizeNote(rawKey[0]);
+    if (NOTE_NAMES.includes(first)) return first;
+  }
+  return 'C';
+};
+
+const safeScale = (scale?: string): string => {
+  if (!scale) return 'Minor';
+  const known = ['major', 'minor', 'phrygian', 'dorian', 'lydian', 'harmonic', 'chromatic'];
+  const lower = scale.toLowerCase();
+  return known.some(k => lower.includes(k)) ? scale : 'Minor';
+};
+
+const evaluateGrooveQuality = (groove: GrooveObject): { score: number; timingTightness: number; harmonicValidity: number } => {
+  let totalNotes = 0;
+  let quantizedNotes = 0;
+  let tonalNotes = 0;
+  let inScaleNotes = 0;
+
+  const key = resolveRootKey(groove.key);
+  const scale = safeScale(groove.scale);
+
+  ELITE_16_CHANNELS.forEach((channel) => {
+    const notes = asArray((groove as any)[channel]);
+    const profile = CHANNEL_PROFILES[channel] || DEFAULT_PROFILE;
+    const tonal = profile.tonal;
+
+    notes.forEach((note) => {
+      totalNotes += 1;
+      const tick = toTick(note);
+      if (tick % profile.quantize === 0) quantizedNotes += 1;
+
+      if (tonal) {
+        tonalNotes += 1;
+        const n = Array.isArray(note.note) ? note.note[0] : note.note;
+        if (typeof n === 'string' && theoryEngine.isNoteInScale(n, key, scale)) inScaleNotes += 1;
+      }
+    });
+  });
+
+  const timingTightness = totalNotes ? quantizedNotes / totalNotes : 1;
+  const harmonicValidity = tonalNotes ? inScaleNotes / tonalNotes : 1;
+  const score = Math.round(((timingTightness * 0.55) + (harmonicValidity * 0.45)) * 100);
+
+  return { score, timingTightness, harmonicValidity };
+};
+
...
+    });
+
+    const metrics = evaluateGrooveQuality(optimized as GrooveObject);
+    optimized.meta = { ...(optimized.meta || {}), qualityMetrics: metrics };

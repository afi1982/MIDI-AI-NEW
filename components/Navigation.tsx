...
import { Home, Zap, Sliders, ListChecks, AudioWaveform, Music, Activity, FileAudio, Cpu, Download } from 'lucide-react';
import { jobQueueService } from '../services/jobQueueService';
import { getEngineStats } from '../services/engineProfileService';
...
export const Navigation: React.FC<NavigationProps> = ({ currentView, onChangeView }) => {
  ...
  const { platform, deferredPrompt, install } = usePwaInstall();
  ...
  const showInstall = platform !== 'standalone' && (platform === 'ios' || !!deferredPrompt);
  ...
  return (
    <header className="h-14 md:h-16 shrink-0 bg-[#08080A] border-b border-white/10 flex items-center justify-between px-3 md:px-6 relative z-[5000] shadow-lg">
      ...
      <nav className="hidden md:flex items-center gap-2 overflow-x-auto no-scrollbar mx-4 h-full scroll-smooth">
        {navItems.map((item) => {
          const isActive = currentView === item.id || (item.id === 'AUDIO_LAB' && currentView === 'TOOLS');
          ...
          return (
            <button
              ...
              className={`
                relative flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 group whitespace-nowrap shrink-0
      ...
      <div className="flex items-center gap-2 shrink-0 ml-2">
        {showInstall && (
          <button
            ...
            className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-sky-500/15 border border-sky-500/30 text-sky-300 text-[9px] font-black uppercase tracking-widest"
          >
            <Download size={12} /> Install
          </button>
        )}
        <div className={`flex items-center gap-2 px-2.5 md:px-3 py-1.5 rounded-xl border transition-all duration-500 ${engineCount > 0 ? 'bg-purple-600/20 border-purple-500/50 text-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.2)]' : 'bg-white/5 border-white/10 text-gray-600'}`}>
